...
# Prediction using FMClassification
startpred=time.time()
prediction, acc, roc, step_size ,factor_size = bgrf.fmclas_prediction(pca_features)
endpred=time.time()
prediction.show()
print("FMClassifier Execution time: %s min" %((endpred-startpred)/60))
