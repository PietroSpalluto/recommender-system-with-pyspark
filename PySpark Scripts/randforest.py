...
startpred=time.time()
prediction, acc, roc, best = bgrf.random_forest_classifier(pca_features)
endpred=time.time()
prediction.show()
print("Random Forest Execution time: %s min" %((endpred-startpred)/60))
print('Best MaxDepth '+str(best._java_obj.parent().getMaxDepth()))
print('Best MinInfoGain '+str(best._java_obj.parent().getMinInfoGain()))
print("NumTrees = " + str(best._java_obj.parent().getNumTrees()))
